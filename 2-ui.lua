...
@kuohaha body part endless ui mobile example, if any bugs contact 
me
...
Menu.Container("Legit", "Resolver", "Right")

Menu.Container("Legit", "Hitbox Expander", "Left")

Menu.Container("Legit", "Trashtalk", "Right")

Menu.Container("Rage", "TargetStrafe", "Left")

Menu.Container("Rage", "Shake", "Right")

Menu.Container("Rage", "Cframe", "Left")

Menu.Container("Visuals", "Fov", "Left")

Menu.Container("Visuals", "Tracer", "Right")
Menu.Container("Visuals", "Texture", "Left")
...
RS.RenderStepped:Connect(function()
    if jamklestarget and jamklestarget.Character and jamklestarget.Character:FindFirstChild(jamkles.camlock.aim_part) then
        ...
        Camera.CFrame = Camera.CFrame:Lerp(lookPosition, jamkles.camlock.smoothness)
...
RunService.RenderStepped:Connect(function()
    for jamklestarget, Line in pairs(Lines) do
        if jamklestarget.Character and jamklestarget.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local ViewportPosition_LocalPlayer = Camera:WorldToViewportPoint(LocalPlayer.Character.HumanoidRootPart.Position)
            local ViewportPosition_jamklestarget, OnScreen_jamklestarget = Camera:WorldToViewportPoint(jamklestarget.Character.HumanoidRootPart.Position)
...
Players.PlayerRemoving:Connect(function(JamklesTarget)
...
Players.PlayerAdded:Connect(function(JamklesTarget)
    if jamklestarget ~= LocalPlayer then
        local Line = Drawing.new("Line")
        Line.Visible = jamkles.Tracer.Visable
        Line.Transparency = jamkles.Tracer.Transparency
    ...
    RS.Heartbeat:Connect(function()
        if jamkles.camlock.Enabled and jamkles.CframeSpeed.Enabled then
            player.Character.HumanoidRootPart.CFrame =
                player.Character.HumanoidRootPart.CFrame + player.Character.Humanoid.MoveDirection * jamkles.CframeSpeed.Speed
        end
    end)
end)
...
RS.RenderStepped:Connect(function()
    ...
    if jamkles.TriggerBot.Enabled and TargetedPlayer and TargetedPlayer.Character and TargetedPlayer.Character:FindFirstChildOfClass("Humanoid").Health ~= 0 then
               ...
               BodyPart.Name == "RightFoot" then
                ...
                if OnScreen then
                    ...
                    if MagnitudeDistance <= FOVCircle.Radius then
                        VirtualInputManager:SendMouseButtonEvent(playerCamera.ViewportSize.X/2, playerCamera.ViewportSize.Y/2, 0, true, Game, 0)
                    end
                end
            end
        end
...
RS.RenderStepped:Connect(function()
    ...
    if jamkles.camlock.Enabled and jamkles.camlock.Toggled and jamklestarget and jamklestarget.Character and jamklestarget.Character:FindFirstChild(jamkles.camlock.aim_part) then
        ...
        local targetPos = aimPart.Position + (jamklestarget.Character.HumanoidRootPart.Velocity * jamkles.camlock.prediction)
...
local cameraLook = CFrame.new(Camera.CFrame.Position, targetPos + shakeOffset)
...
RS.RenderStepped:Connect(function()
    if jamklestarget and jamklestarget.Character and jamklestarget.Character:FindFirstChild(jamkles.camlock.aim_part) then
        ...
        local shakeOffset = Vector3.new(
            shakeEnabled and math.random(-getgenv().jamkles.Shake.X, getgenv().jamkles.Shake.X) * 0.1 or 0,
            shakeEnabled and math.random(-getgenv().jamkles.Shake.Y, getgenv().jamkles.Shake.Y) * 0.1 or 0,
            shakeEnabled and math.random(-getgenv().jamkles.Shake.Z, getgenv().jamkles.Shake.Z) * 0.1 or 0
        )

        local aimPart = jamklestarget.Character[jamkles.camlock.aim_part]
        local predictedPosition = aimPart.Position + (jamklestarget.Character.HumanoidRootPart.Velocity * jamkles.camlock.prediction)
...
Game:GetService("RunService").RenderStepped:Connect(function()
    if jamkles.Reach.Enabled then
        for index, player in pairs(Game:GetService("Players"):GetPlayers()) do
            if player ~= Game:GetService("Players").LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChildOfClass("Humanoid")and player.Character:FindFirstChildOfClass("Humanoid").Health > 2.5 and Game:GetService("Players").LocalPlayer.Character and Game:GetService("Players").LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local PlayerHumanoidRootPart = Game:GetService("Players").LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                ...
                if jamkles.BringAll.Enabled then
                    ...
                    TargetHumanoidRootPart.CFrame = CFrame.new(PlayerHumanoidRootPart.CFrame.Position - (PlayerHumanoidRootPart.CFrame.LookVector * -jamkles.BringAll.Distance), PlayerHumanoidRootPart.CFrame.Position)
                end
            end
        end
...
local function resolveTargetVelocity(jamklestarget)
    if getgenv().jamkles.Resolver.Enabled then
        if getgenv().jamkles.Resolver.Method == "RecalculateVelocity" then
            ...
            if lastPosition and jamklestarget.Character:FindFirstChild(getgenv().jamkles.Resolver.PredictionSettings.HitPart) then
                local resolvedVelocity = (jamklestarget.Character[getgenv().jamkles.Resolver.PredictionSettings.HitPart].Position - lastPosition) / deltaTime
                lastPosition = jamklestarget.Character[getgenv().jamkles.Resolver.PredictionSettings.HitPart].Position
